...
dimensions (
    ...
    -- Client Information
    CLIENTS.VERTICAL as client_type with synonyms=('client segment','organization type','provider type') comment='Type of healthcare organization (Retail, Tech, Manufacturing mapped to DME, Hospital, etc)',
...
metrics (
    ...
    TICKETS.OPEN_TICKETS as COUNT(CASE WHEN tickets.status IN ('Open', 'In Progress') THEN 1 END) comment='Number of open/in-progress tickets',
    TICKETS.RESOLVED_TICKETS as COUNT(CASE WHEN tickets.status = 'Resolved' THEN 1 END) comment='Number of resolved tickets',
    ...
    TICKETS.ESCALATION_RATE as (COUNT(CASE WHEN tickets.escalated_flag THEN 1 END) / COUNT(*) * 100) comment='Percentage of tickets that were escalated',
    TICKETS.RESOLUTION_RATE as (COUNT(CASE WHEN tickets.status = 'Resolved' THEN 1 END) / COUNT(*) * 100) comment='Percentage of tickets resolved',
    
    -- Trend Metrics
    TICKETS.TICKETS_BY_PRODUCT_AREA as COUNT(*) comment='Number of tickets by product area for ranking',
    TICKETS.HIGH_SEVERITY_TICKETS as COUNT(CASE WHEN tickets.severity_level IN ('Critical', 'High') THEN 1 END) comment='Number of high severity tickets',
